...
- **Realtime backend:** [partyserver](https://github.com/cloudflare/partykit) on Cloudflare Workers + Durable Objects — one DO per room
...
npm install
npm run dev:all            # vite (5173) + wrangler (1999) concurrently
```
...
echo "$SHARED_PW" | npx wrangler secret put APP_PASSWORD
...
| Var | Value |
|---|---|
| `VITE_PARTYKIT_HOST` | `suspect-game-server.<account>.workers.dev` (the URL printed by `wrangler deploy`) |
| `VITE_APP_PASSWORD` | The same string you pushed as `APP_PASSWORD` to the worker |

**Custom subdomain:** Vercel project → Settings → Domains → add e.g. `suspect.yourdomain.com`.

> **Auth model:** The password is validated by the Worker DO before a join is accepted, *and* by the client gate as a UX nicety. Even if someone bypasses the client gate via devtools, the server rejects their WebSocket join.

## Structure

```
worker/
  index.ts             # Worker entry: routes /parties/main/:room via routePartykitRequest
  server.ts            # GameServer DO: state machine, scoring, timers, password check
src/
